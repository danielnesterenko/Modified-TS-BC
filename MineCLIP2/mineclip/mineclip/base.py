...
class VideoRewardBase(nn.Module):
    ...
    def forward_image_features(self, frames):
        ...
        #print('1')
        #print(frames.shape)
        features, patch_embeddings = self.image_encoder(frames)
        #print('DEBUG01')
        #print(features.shape)
        #print(patch_embeddings.shape)
        output = features.view(*leading_dims, features.size(-1))
        #print('FORWARD IMAGE FEATURES OUTPUT: ')
        #print(output.shape)
        return output, patch_embeddings

    def forward_video_features(self, image_features):
        #print("FORWARD VIDEO FEATURES")
        """
        [B, L, F] -> [B, F]
        """
        image_features, patch_embeddings = image_features
        #print(image_features.shape)
        B, L, F = image_features.size()
        video_feats = self.temporal_encoder(image_features)
        assert video_feats.shape[0] == B
        #print('FORWARD VIDEO FEATURES OUTPUT')
        #print(video_feats.shape)
        return video_feats, patch_embeddings
